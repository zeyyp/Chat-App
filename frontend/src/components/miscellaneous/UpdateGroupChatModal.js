...
const UpdateGroupChatModal = ({ fetchAgain, setFetchAgain, fetchMessages }) => {
  ...
  const handleRemove = async (user1) => {
    // Sadece admin birini silebilir veya kullanıcı kendisi gruptan çıkabilir
    ...
    try {
      ...
      // fetchMessages(); // Mesajlar kısmında eklenecek
      setLoading(false);
    } catch (error) {
    ...
    <>
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <ModalContent>
          ...
          <ModalFooter><Button onClick={() => handleRemove(user)} colorScheme="red">Leave Group</Button></ModalFooter>
        </ModalContent>
      </Modal>
    </>
  );
